...
# On Windows: default JAVA_HOME to Android Studio JBR if not already set
ifeq ($(OS),Windows_NT)
...
SDK_DIR_FROM_LOCAL := $(shell [ -f $(LOCAL_PROPERTIES) ] && sed -n 's/^sdk\.dir=//p' $(LOCAL_PROPERTIES) | sed 's/\\:/:/g; s/\\\\/\//g' | tail -n 1)
